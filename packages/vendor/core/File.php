...
class File
{
     ...
     * @access public
     */
    public static function getCount($path) : int
    {
        $folder = true;
        $size   = 0;
        $ignore = array('.','..');
     ...
     * @access private
     */
    private static function getMimeType($path) : string
    {
        $type = array(
            'css'   => 'text/css',
        ...
        return $type[pathinfo($path, PATHINFO_EXTENSION)];
    }
     ...
     * @access private
     */
    private static function serveStaticFile($path, $options = array()) : bool
    {
        if (session_id()) {
            session_write_close();
        }
